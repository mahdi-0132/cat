...
function SuggestionMoviesItems() {
  const [movieImage, setMovieImage] = useState<IHomeSuggestion[]>([])
  ...
  useEffect(() => {
    fetch("http://localhost:3001/suggestions")
      .then((res) => res.json())
      .then((data) => setMovieImage(data.movies))
  }, [])

const chunckSize = 5
const slides = []
for(let i = 0 ; i< movieImage.length; i += chunckSize ){
  slides.push(movieImage.slice(i,i + chunckSize))
}
              ...
              {group.map((suggece, index) => (
                ...
                <div className='relative group flex flex-col items-center' key={index}> 
                    ...
                    <img
                      ...
                      <div className='flex bg-neutral-800 px-20 py-1 rounded-lg space-x-3'>
                        <p> {suggece.runtime} </p>
                        <img src="/clock.png" width={10} height={10} />
    
                           <p> {suggece.imdb_rating} </p>
...
export default SuggestionMoviesItems